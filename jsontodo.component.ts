...
@Component({
  ...
  styleUrl: './jsontodo.component.scss',
...
export class JsontodoComponent implements OnInit {
  ...
  // this below onSubmit code for the mongodb database

  onSubmit(): void {
    if (this.task.trim()) {
        const newTodo = { title: this.task.trim(), completed: false };
        if (this.editingIndex !== null && this.editingIndex >= 0) {
            const id = this.tasks[this.editingIndex]._id; 
            this.loading = true;
            this.errorMessage = ''; 
            this.api.updateTodo(id, newTodo).subscribe(
                (updatedTodo) => {
  ...
  onDelete(index: number): void {
    ...
    if (userConfirmed) {
      // const id = this.tasks[index].id;  // this is for the json placeholder api
      const id = this.tasks[index]._id; //this is for the mongodb database id
      this.loading = true; // Show spinner
      this.errorMessage = ''; 
